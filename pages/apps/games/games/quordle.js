// Quordle Game (4 Wordle boards)

class QuordleGame extends GameEngine {
    constructor() {
        super('quordle');
        this.words = [];
        this.guesses = [];
    ...
    init() {
        ...
        container.innerHTML = `
            <div class="word-game-container">
                <div class="word-boards quordle">
                    <div id="qboard-1" class="word-board"></div>
                    <div id="qboard-2" class="word-board"></div>
                    <div id="qboard-3" class="word-board"></div>
                    <div id="qboard-4" class="word-board"></div>
                </div>
                <div class="game-keyboard" id="qkeyboard"></div>
            </div>
        `;

        this.words = [
            this.randomChoice(WORD_LIST),
            this.randomChoice(WORD_LIST),
            this.randomChoice(WORD_LIST),
            this.randomChoice(WORD_LIST)
        ];
        ...
        document.getElementById('game-instructions').innerHTML = `
            <h3>How to Play</h3>
            <p>Guess all four 5-letter words in 9 tries</p>
            <p>Green = correct, Yellow = wrong position, Gray = not in word</p>
        `;
    }

    renderBoards() {
        for (let boardNum = 1; boardNum <= 4; boardNum++) {
            const board = document.getElementById(`qboard-${boardNum}`);
            board.innerHTML = '';
    ...
    renderKeyboard() {
        const keyboard = document.getElementById('qkeyboard');
        const rows = [
            ['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P'],
            ['A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L'],
            ['ENTER', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', '⌫']
    ...
    updateDisplay() {
        for (let boardNum = 1; boardNum <= 4; boardNum++) {
            for (let i = 0; i < 5; i++) {
                const tile = document.getElementById(`qtile-${boardNum}-${this.currentRow}-${i}`);
    ...
    colorTiles(boardNum, row, guess, word) {
        for (let i = 0; i < 5; i++) {
            const tile = document.getElementById(`qtile-${boardNum}-${row}-${i}`);
...
let quordleGame = null;

function initQuordle() {
    quordleGame = new QuordleGame();
    quordleGame.init();
}
